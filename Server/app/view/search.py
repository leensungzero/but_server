...
class AllUserView(BaseResource):
    ...
    def get(self):
        ...
        return [{
            ...
            'user_name': user.name,
            'tag': [
                personality_type[PersonalityModel.get_personality(user.id)],
                attention_type[AttentionModel.get_attention(user.id)],
                character_type[CharacterModel.get_character(user.id)]
            ]
        } for user in user_list], 200
